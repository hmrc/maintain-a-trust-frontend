...
class TrustStatusController @Inject() (
  ...
  def closed(): Action[AnyContent] = actions.authWithOptionalData.async { implicit request =>
    Future.successful(Ok(closedView(request.user.affinityGroup, request.identifier, request.identifierType)))
  }

  def processing(): Action[AnyContent] = actions.authWithOptionalData.async { implicit request =>
    Future.successful(Ok(stillProcessingView(request.user.affinityGroup, request.identifier, request.identifierType)))
  }

  def sorryThereHasBeenAProblem(): Action[AnyContent] = actions.authWithOptionalData.async { implicit request =>
    Future.successful(Ok(playbackProblemContactHMRCView(request.identifier, request.identifierType)))
  }

  def notFound(): Action[AnyContent] = actions.authWithOptionalData.async { implicit request =>
    Future.successful(
      Ok(identifierDoesNotMatchView(request.user.affinityGroup, request.identifier, request.identifierType))
    )
  }

  def locked(): Action[AnyContent] = actions.authWithOptionalData.async { implicit request =>
  ...
  def status(): Action[AnyContent] = actions.authWithOptionalData.async { implicit request =>
    checkIfLocked(request.identifier, fromVerify = false)
  }

  def statusAfterVerify(): Action[AnyContent] = actions.authWithOptionalData.async { implicit request =>
    checkIfLocked(request.identifier, fromVerify = true)
  }

  private def checkIfLocked(identifier: String, fromVerify: Boolean)(implicit
    request: OptionalDataRequest[AnyContent]
  ): Future[Result] =
    trustStoreConnector.get(identifier).value.flatMap {
      case Right(Some(claim)) if claim.trustLocked =>
        logger.warn(
          s"[$className][checkIfLocked][Session ID: ${Session.id(hc)}] $identifier user has failed IV 3 times, locked out for 30 minutes"
        )
        Future.successful(Redirect(controllers.routes.TrustStatusController.locked()))
      case Right(_)                                =>
        logger.info(
          s"[$className][checkIfLocked][Session ID: ${Session.id(hc)}] $identifier user has not been locked out from IV"
        )
        tryToPlayback(identifier, fromVerify)
      case Left(_)                                 =>
        logger.warn(s"[$className][checkIfLocked][Session ID: ${Session.id(hc)}] Errors from connector call.")
        errorHandler.internalServerErrorTemplate.map(InternalServerError(_))
    }

  private def tryToPlayback(identifier: String, fromVerify: Boolean)(implicit
    request: OptionalDataRequest[AnyContent]
  ): Future[Result] =
    trustConnector.playbackFromEtmp(identifier).value.flatMap {
      case Right(Closed)                      =>
        logger.info(
          s"[$className][tryToPlayback][Session ID: ${Session.id(hc)}] $identifier unable to retrieve trust due it being closed"
        )
        Future.successful(Redirect(controllers.routes.TrustStatusController.closed()))
      case Right(Processing)                  =>
        logger.info(
          s"[$className][tryToPlayback][Session ID: ${Session.id(hc)}] $identifier unable to retrieve trust due to trust change processing"
        )
        Future.successful(Redirect(controllers.routes.TrustStatusController.processing()))
      case Right(IdentifierNotFound)          =>
        logger.info(
          s"[$className][tryToPlayback][Session ID: ${Session.id(hc)}] $identifier unable to retrieve trust due to UTR/URN not found"
        )
      ...
      case Right(SorryThereHasBeenAProblem)   =>
        logger.warn(
          s"[$className][tryToPlayback][Session ID: ${Session.id(hc)}] $identifier unable to retrieve trust due no content returned"
        )
        Future.successful(Redirect(routes.TrustStatusController.sorryThereHasBeenAProblem()))
      case Right(TrustServiceUnavailable)     =>
        logger.error(
          s"[$className][tryToPlayback][Session ID: ${Session.id(hc)}] $identifier unable to retrieve trust due to the service being unavailable"
        )
        Future.successful(Redirect(routes.TrustStatusController.down()))
      case Right(ClosedRequestResponse)       =>
        logger.error(
          s"[$className][tryToPlayback][Session ID: ${Session.id(hc)}] $identifier unable to retrieve trust due to the connection timing out"
        )
        Future.successful(Redirect(routes.TrustStatusController.down()))
      case Right(TrustsErrorResponse(status)) =>
        logger.error(
          s"[$className][tryToPlayback][Session ID: ${Session.id(hc)}] $identifier unable to retrieve trust due to an unexpected response $status"
        )
        Future.successful(Redirect(routes.TrustStatusController.down()))
      case Left(response)                     =>
        logger.error(
          s"[$className][tryToPlayback][Session ID: ${Session.id(hc)}] $identifier unable to retrieve trust due to an error $response"
        )
        Future.successful(Redirect(routes.TrustStatusController.down()))
    }
  ...
  private def authenticateForIdentifierAndExtract(identifier: String, playback: GetTrust, fromVerify: Boolean)(implicit
    ...
    val expectedResult = for {
      ...
      _                              = if (trustDetails.isTaxable && IdentifierType(identifier) == URN) {
                                         logger.warn(
    ...
    expectedResult.value.map {
      ...
      case Left(_) =>
        logger.warn(
          s"[$className][authenticateForIdentifierAndExtract][Session ID: ${Session.id(hc)}]" + "authentication and extraction failed."
        )
  ...
  private def extract(
    ...
    logger.info(
      s"[$className][extract][Session ID: ${Session.id(hc)}] user authenticated for $identifier, attempting to extract to user answers"
    )
    ...
    playbackExtractor.extract(userAnswers, playback).value flatMap {
      case Right(answers) =>
        playbackRepository.set(answers).value map { _ =>
          logger.info(
            s"[$className][extract][Session ID: ${Session.id(hc)}] $identifier successfully extracted, showing information about maintaining"
          )
          routeAfterExtraction(userAnswers, fromVerify, trustDetails)
        }
      case Left(reason)   =>
        logger.warn(
          s"[$className][extract][Session ID: ${Session.id(hc)}] $identifier unable to extract user answers due to $reason"
        )
        Future.successful(Redirect(routes.TrustStatusController.sorryThereHasBeenAProblem()))
    }
  }

  private def routeAfterExtraction[A](answers: UserAnswers, fromVerify: Boolean, trustDetails: TrustDetails)(implicit
    ...
    if (answers.trustMldStatus == Underlying4mldTrustIn5mldMode) {
      logger.info(
        s"[$className][routeAfterExtraction][Session ID: ${Session.id(hc)}] underlying data is 4MLD. " +
      ...
      logger.info(
        s"[$className][routeAfterExtraction][Session ID: ${Session.id(hc)}] User has not answered Schedule3a question " +
