...
import forms.WhatIsNext5mldFormProvider
import models.Enumerable
import models.pages.WhatIsNext5mld
import pages.WhatIsNext5mldPage
import play.api.data.Form
import play.api.i18n.MessagesApi
import play.api.mvc.{Action, AnyContent, MessagesControllerComponents}
import repositories.PlaybackRepository
import views.html.WhatIsNext5mldView

import scala.concurrent.{ExecutionContext, Future}

@Singleton
class WhatIsNext5mldController @Inject()(
                                      override val messagesApi: MessagesApi,
                                      playbackRepository: PlaybackRepository,
                                      actions: Actions,
                                      formProvider: WhatIsNext5mldFormProvider,
                                      val controllerComponents: MessagesControllerComponents,
                                      view: WhatIsNext5mldView,
  ...
  val form: Form[WhatIsNext5mld] = formProvider()

  def onPageLoad(): Action[AnyContent] = actions.verifiedForIdentifier {
    implicit request =>

      val preparedForm = request.userAnswers.get(WhatIsNext5mldPage) match {
        case None => form
  ...
  def onSubmit(): Action[AnyContent] = actions.verifiedForIdentifier.async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value => {
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(WhatIsNext5mldPage, value))
            _ <- playbackRepository.set(updatedAnswers)
          } yield value match {
            case WhatIsNext5mld.DeclareTheTrustIsUpToDate =>
              redirectToDeclaration()

            case WhatIsNext5mld.MakeChanges =>
              Redirect(controllers.makechanges.routes.UpdateTrusteesYesNoController.onPageLoad())

            case WhatIsNext5mld.CloseTrust if config.closeATrustEnabled =>
              Redirect(controllers.close.routes.DateLastAssetSharedOutYesNoController.onPageLoad())

            case WhatIsNext5mld.NoLongerTaxable =>
