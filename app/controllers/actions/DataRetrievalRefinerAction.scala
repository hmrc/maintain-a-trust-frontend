...
@Singleton
class DataRetrievalRefinerAction @Inject() (
  ...
  private def createdOptionalDataRequest[A](
  ...
  private def handlePlaybackRepositoryResponse[A](request: IdentifierRequest[A], session: IdentifierSession)(implicit
    hc: HeaderCarrier
    ...
    playbackRepository.get(request.user.internalId, session.identifier, Session.id(hc)).value.flatMap {
      case Right(None)              =>
        logger.info(
          s"[$className][handlePlaybackRepositoryResponse] no user answers in session for UTR/URN ${session.identifier}"
        )
        Future.successful(Right(createdOptionalDataRequest(request, None, session.identifier)))
      case Right(Some(userAnswers)) =>
        logger.info(
          s"[$className][handlePlaybackRepositoryResponse] user answers found in session for UTR/URN ${session.identifier}"
        )
        Future.successful(Right(createdOptionalDataRequest(request, Some(userAnswers), session.identifier)))
      case Left(_)                  =>
        logger.warn(
          s"[$className][handlePlaybackRepositoryResponse] Error while retrieving data from playback repository"
        )
        errorHandler.internalServerErrorTemplate(request.request).map { html =>
          Left(InternalServerError(html))
        }
