...
class RefreshedDataPreSubmitRetrievalActionImpl @Inject() (
  ...
  override def refine[A](request: DataRequest[A]): Future[Either[Result, DataRequest[A]]] = {
    ...
    val expectedResult = for {
      ...
      submissionData           =
        SubmissionData(identifier, whatIsNext, optionalAgentInformation, getClosureDate(request.userAnswers))
  ...
  private def handlePlaybackResponse[A](trustsResponse: TrustsResponse, submissionData: SubmissionData)(implicit
