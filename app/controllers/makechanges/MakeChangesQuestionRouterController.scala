...
abstract class MakeChangesQuestionRouterController(
  ...
  private def redirectAndResetTaskList(
    updatedAnswers: UserAnswers
  )(implicit request: DataRequest[AnyContent], hc: HeaderCarrier): TrustEnvelope[Result] = EitherT {
    trustStoreConnector
      .set(request.userAnswers.identifier, updatedAnswers)
      .value
      .map(_.map(_ => Redirect(controllers.tasklist.routes.TaskListController.onPageLoad())))
  }
  ...
  protected def routeToAddOrUpdateNonEeaCompany(updatedAnswers: UserAnswers, isClosingTrust: Boolean)(implicit
  ...
  protected def routeToDeclareOrTaskList(updatedAnswers: UserAnswers, isClosingTrust: Boolean)(implicit
    request: DataRequest[AnyContent]
  ): TrustEnvelope[Result] =
