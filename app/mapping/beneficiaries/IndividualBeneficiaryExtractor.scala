...
class IndividualBeneficiaryExtractor extends BeneficiaryPlaybackExtractor[DisplayTrustIndividualDetailsType] {
  ...
  override def passportOrIdCardPage(index: Int): QuestionPage[PassportType] = IndividualBeneficiaryPassportIDCardPage(
    index
  )

  override def dateOfBirthYesNoPage(index: Int): QuestionPage[Boolean] = IndividualBeneficiaryDateOfBirthYesNoPage(
    index
  )
  ...
  private def extractRoleInCompany(
    individualBeneficiary: DisplayTrustIndividualDetailsType,
    index: Int,
    answers: UserAnswers
  ): Either[TrustErrors, UserAnswers] =
  ...
  private def extractVulnerability(
    vulnerable: Option[Boolean],
    index: Int,
    answers: UserAnswers
  ): Either[TrustErrors, UserAnswers] =
    extractIfTaxableOrMigratingToTaxable(answers) {
      vulnerable match {
        ...
        case None        =>
          if (answers.isTrustMigratingFromNonTaxableToTaxable) {
            logger.warn(
              s"[IndividualBeneficiaryExtractor][extractVulnerability][UTR/URN: ${answers.identifier}] Individual beneficiary vulnerability should be answered as part of trust becoming taxable."
            )
          } else {
            logger.warn(
              s"[IndividualBeneficiaryExtractor][extractVulnerability][UTR/URN: ${answers.identifier}] Individual beneficiary vulnerability must be answered for taxable trust."
            )
          }
