...
trait TrusteePlaybackExtractor[T <: TrusteeType] extends PlaybackExtractor[T] {
  ...
  override def countryOfNationalityYesNoPage(index: Int): QuestionPage[Boolean] = TrusteeCountryOfNationalityYesNoPage(
    index
  )
  ...
  override def countryOfResidenceYesNoPage(index: Int): QuestionPage[Boolean] = TrusteeCountryOfResidenceYesNoPage(
    index
  )
  ...
  def extractEmail(email: Option[String], index: Int, answers: UserAnswers): Either[TrustErrors, UserAnswers] =
    email match {
      case Some(value) =>
        answers
          .set(TrusteeEmailYesNoPage(index), true)
