...
import sections.assets.OtherAsset
...
class OtherAssetPrinter @Inject()(converter: AnswerRowConverter) extends EntitiesPrinter[JsArray] with EntityPrinter[String] {
  ...
  override def namePath(index: Int): JsPath = OtherAssetDescriptionPage(index).path

  override def section: QuestionPage[JsArray] = OtherAsset

  override def headingKey(migratingFromNonTaxableToTaxable: Boolean): Option[String] = None

  override val subHeadingKey: Option[String] = Some("asset.other")
