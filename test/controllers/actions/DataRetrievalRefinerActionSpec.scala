...
class DataRetrievalRefinerActionSpec extends SpecBase with MockitoSugar with ScalaFutures {
  ...
  class Harness()
      extends DataRetrievalRefinerAction(mockSessionRepository, mockPlaybackRepository, mockErrorHandler, appConfig)(
        executionContext
      ) {
  ...
  "Data Retrieval Action" when {
    ...
    "there is no data in the cache" must {
      ...
      "set userAnswers to 'None' in the request" in {
        ...
        when(mockSessionRepository.get("id"))
          .thenReturn(
            EitherT[Future, TrustErrors, Option[IdentifierSession]](
              Future.successful(Right(Some(IdentifierSession("id", "utr"))))
            )
          )
    ...
    "there is data in the cache" must {
      ...
      "build a userAnswers object and add it to the request" in {
        ...
        when(mockPlaybackRepository.get("id", "utr", sessionId))
          .thenReturn(
            EitherT[Future, TrustErrors, Option[UserAnswers]](
              Future.successful(Right(Some(TestUserAnswers.emptyUserAnswersForUtr)))
            )
          )
