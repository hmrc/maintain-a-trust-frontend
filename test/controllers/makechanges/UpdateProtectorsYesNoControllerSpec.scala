...
class UpdateProtectorsYesNoControllerSpec extends SpecBase {
  ...
  private lazy val updateProtectorYesNoRoute: String = routes.UpdateProtectorYesNoController.onPageLoad().url

  "UpdateProtectorYesNoController" when {
    ...
    "making changes" must {
      ...
      "redirect to the add an other individuals page when valid data is submitted and no individuals exist" in {
        ...
        val request = FakeRequest(POST, updateProtectorYesNoRoute)
