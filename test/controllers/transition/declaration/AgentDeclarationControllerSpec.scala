...
import models.errors.{DeclarationError, MongoError, TrustErrors}
    ...
    "redirect to confirmation for a POST" in {
      ...
      val application =
        applicationBuilder(
          ...
          bind[DeclarationService].to(mockDeclarationService)
        ).build()
      ...
      redirectLocation(result).value mustBe controllers.transition.declaration.routes.ConfirmationController
        .onPageLoad()
        .url
